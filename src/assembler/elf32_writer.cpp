...
std::vector<uint8_t> ELF32ObjectWriter::write_object(
    ...
    for (const auto& symbol : program.symbols) {
        ...
        write_symbol(name_offset, static_cast<uint32_t>(symbol.offset), static_cast<uint32_t>(symbol.size), static_cast<uint8_t>((symbol_binding(symbol.binding) << 4) | sym_type), 0, shndx);
    ...
    // .comment section with assembler version info
    ...
    sections.push_back({".comment", 0, SHT_PROGBITS, 0, 0, static_cast<uint32_t>(comment_data.size()), 0, 0, 1, 0, comment_data});
