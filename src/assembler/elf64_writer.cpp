...
constexpr uint16_t EHDR_SIZE = 64;
constexpr uint16_t SHDR_SIZE = 64;
...
std::vector<uint8_t> emit_section_payload(const IRProgram& program, IRSectionKind section) {
    ...
    for (const auto& record : program.data_records) {
        ...
        if (record.directive == ".dw") {
            for (const auto& value : record.values) {
                write16(out, static_cast<uint16_t>(std::get<IRImmediateOperand>(value.value).value));
            }
            continue;
        }
        ...
        if (record.directive == ".dq") {
            for (const auto& value : record.values) {
                write64(out, static_cast<uint64_t>(std::get<IRImmediateOperand>(value.value).value));
            }
            continue;
        }
...
std::vector<uint8_t> ELF64ObjectWriter::write_object(
    const IRProgram& program,
    std::vector<std::string>& errors,
    const std::vector<uint8_t>* text_bytes,
    const std::vector<IRRelocation>* text_relocations) {
    if (program.target != IRTarget::X86Elf64) {
        errors.push_back("ELF64 object writer requires IR target x86-elf64");
        return {};
    }

    if (!program.instructions.empty() && text_bytes == nullptr) {
        errors.push_back("ELF64 object writer currently requires backend-supplied text bytes");
        return {};
    }
    ...
    sections.back().size = sections.back().data.size();
    const uint32_t data_index = static_cast<uint32_t>(sections.size());
    sections.push_back({".data", 0, SHT_PROGBITS, SHF_ALLOC | SHF_WRITE, 0, 0, 0, 0, 8, 0, emit_section_payload(program, IRSectionKind::Data)});
    sections.back().size = sections.back().data.size();
    ...
    sections.push_back({".bss", 0, SHT_NOBITS, SHF_ALLOC | SHF_WRITE, 0, static_cast<uint64_t>(compute_bss_size(program)), 0, 0, 8, 0, {}});

    std::vector<uint8_t> strtab(1, '\0');
    std::vector<uint8_t> symtab;
    std::vector<uint8_t> rela_text;
    std::vector<uint8_t> rela_data;
    std::vector<uint8_t> rela_rodata;
    std::vector<uint8_t> shstrtab(1, '\0');

    auto write_symbol = [&](uint32_t name, uint8_t info, uint8_t other, uint16_t shndx, uint64_t value, uint64_t size) {
        write32(symtab, name);
        symtab.push_back(info);
        symtab.push_back(other);
        write16(symtab, shndx);
        write64(symtab, value);
        write64(symtab, size);
    };

    write_symbol(0, 0, 0, 0, 0, 0);
    write_symbol(0, static_cast<uint8_t>((STB_LOCAL << 4) | STT_SECTION), 0, static_cast<uint16_t>(text_index), 0, 0);
    write_symbol(0, static_cast<uint8_t>((STB_LOCAL << 4) | STT_SECTION), 0, static_cast<uint16_t>(data_index), 0, 0);
    write_symbol(0, static_cast<uint8_t>((STB_LOCAL << 4) | STT_SECTION), 0, static_cast<uint16_t>(rodata_index), 0, 0);
    write_symbol(0, static_cast<uint8_t>((STB_LOCAL << 4) | STT_SECTION), 0, static_cast<uint16_t>(bss_index), 0, 0);
    ...
    for (const auto& symbol : program.symbols) {
        ...
        write_symbol(name_offset, static_cast<uint8_t>((symbol_binding(symbol.binding) << 4) | STT_NOTYPE), 0, shndx, symbol.offset, 0);
    ...
    auto append_relocation = [&](std::vector<uint8_t>& out, const IRRelocation& relocation) {
        ...
        switch (relocation.kind) {
            case IRRelocationKind::Absolute64:
        ...
        write64(out, relocation.offset);
        write64(out, (static_cast<uint64_t>(it->second) << 32) | relocation_type);
        write64(out, static_cast<uint64_t>(relocation.addend));
    };

    if (text_relocations != nullptr) {
        for (const auto& relocation : *text_relocations) {
            append_relocation(rela_text, relocation);
        }
    }
    ...
    for (const auto& relocation : program.relocations) {
        switch (relocation.section) {
            ...
            case IRSectionKind::Data:
                append_relocation(rela_data, relocation);
                break;
            case IRSectionKind::Rodata:
                append_relocation(rela_rodata, relocation);
                break;
            default:
                break;
        }
    ...
    const uint32_t rela_text_index = static_cast<uint32_t>(sections.size());
    sections.push_back({".rela.text", 0, SHT_RELA, 0, 0, static_cast<uint64_t>(rela_text.size()), 0, text_index, 8, 24, rela_text});
    const uint32_t rela_data_index = static_cast<uint32_t>(sections.size());
    sections.push_back({".rela.data", 0, SHT_RELA, 0, 0, static_cast<uint64_t>(rela_data.size()), 0, data_index, 8, 24, rela_data});
    const uint32_t rela_rodata_index = static_cast<uint32_t>(sections.size());
    sections.push_back({".rela.rodata", 0, SHT_RELA, 0, 0, static_cast<uint64_t>(rela_rodata.size()), 0, rodata_index, 8, 24, rela_rodata});
    const uint32_t symtab_index = static_cast<uint32_t>(sections.size());
    sections.push_back({".symtab", 0, SHT_SYMTAB, 0, 0, static_cast<uint64_t>(symtab.size()), 0, first_global_symbol_index, 8, 24, symtab});
    const uint32_t strtab_index = static_cast<uint32_t>(sections.size());
    sections.push_back({".strtab", 0, SHT_STRTAB, 0, 0, static_cast<uint64_t>(strtab.size()), 0, 0, 1, 0, strtab});
    ...
    sections[shstrtab_index].size = shstrtab.size();
    sections[symtab_index].link = strtab_index;
    sections[rela_text_index].link = symtab_index;
    sections[rela_data_index].link = symtab_index;
    sections[rela_rodata_index].link = symtab_index;
    ...
    elf[18] = static_cast<uint8_t>(EM_X86_64 & 0xFF);
    elf[19] = static_cast<uint8_t>((EM_X86_64 >> 8) & 0xFF);
    elf[20] = static_cast<uint8_t>(EV_CURRENT & 0xFF);
    elf[52] = static_cast<uint8_t>(EHDR_SIZE & 0xFF);
    elf[53] = static_cast<uint8_t>((EHDR_SIZE >> 8) & 0xFF);
    elf[58] = static_cast<uint8_t>(SHDR_SIZE & 0xFF);
    elf[59] = static_cast<uint8_t>((SHDR_SIZE >> 8) & 0xFF);
    elf[60] = static_cast<uint8_t>(sections.size() & 0xFF);
    elf[61] = static_cast<uint8_t>((sections.size() >> 8) & 0xFF);
    elf[62] = static_cast<uint8_t>(shstrtab_index & 0xFF);
    elf[63] = static_cast<uint8_t>((shstrtab_index >> 8) & 0xFF);
    ...
    for (size_t index = 1; index < sections.size(); ++index) {
        ...
        file_offset = align_to(file_offset, static_cast<size_t>(section.addralign));
        section.offset = file_offset;
        file_offset += section.data.size();
    }

    const uint64_t shoff = align_to(file_offset, 8);
    for (int i = 0; i < 8; ++i) {
        elf[40 + i] = static_cast<uint8_t>((shoff >> (i * 8)) & 0xFF);
    }

    elf.resize(static_cast<size_t>(shoff), 0);
    for (const auto& section : sections) {
        if (section.type == SHT_NOBITS || section.data.empty()) {
            continue;
        }
        if (elf.size() < section.offset + section.data.size()) {
            elf.resize(static_cast<size_t>(section.offset + section.data.size()), 0);
        }
        std::copy(section.data.begin(), section.data.end(), elf.begin() + static_cast<std::ptrdiff_t>(section.offset));
    }

    elf.resize(static_cast<size_t>(shoff));
    for (const auto& section : sections) {
        write32(elf, section.name_offset);
        write32(elf, section.type);
        write64(elf, section.flags);
        write64(elf, 0);
        write64(elf, section.offset);
        write64(elf, section.size);
        write32(elf, section.link);
        write32(elf, section.info);
        write64(elf, section.addralign);
        write64(elf, section.entsize);
    }
